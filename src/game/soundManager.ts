...
// Create simple beep sounds using Howler
const createBeepSound = (frequency: number, duration: number = 100) => {
  return new Howl({
    src: [generateBeepDataUrl(frequency, duration)],
    volume: 0.3,
    preload: true,
  })
}

const generateBeepDataUrl = (frequency: number, duration: number) => {
  ...
  return audioContext.createMediaElementAudioSourceOrGetUserMedia
    ? dataUrlFromAudioContext(audioContext)
    : ''
}

const dataUrlFromAudioContext = (ctx: AudioContext): string => {
